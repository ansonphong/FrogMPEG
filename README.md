...
| `social-reel-60` | 1080×1920 | 60 | 24M | `h264-nvenc-mp4`, `fit: pad`, faststart |
| `social-reel-30` | 1080×1920 | 30 | 24M | `h264-nvenc-mp4`, `fit: pad`, faststart |
| `social-reel-fill-60` | 1080×1920 | 60 | 24M | `h264-nvenc-mp4`, `fit: crop`, faststart |
| `social-reel-fill-30` | 1080×1920 | 30 | 24M | `h264-nvenc-mp4`, `fit: crop`, faststart |
| `social-feed-60` | 1080×1350 | 60 | 24M | `h264-nvenc-mp4`, `fit: pad`, faststart |
| `social-feed-30` | 1080×1350 | 30 | 24M | `h264-nvenc-mp4`, `fit: pad`, faststart |
| `social-square-60` | 1080×1080 | 60 | 24M | `h264-nvenc-mp4`, faststart |
| `social-square-30` | 1080×1080 | 30 | 24M | `h264-nvenc-mp4`, faststart |

Each shape has a 30 fps preset and a 60 fps preset. The 30 fps name keeps the timing of a 30 fps image sequence. The 60 fps name plays those same frames at 60 fps, so a 2-minute sequence becomes 1 minute. `social-reel`, `social-reel-fill`, `social-feed`, and `social-square` are the same settings as the matching `-60` preset.

`social-reel-60` is the file for Instagram Reels, Stories, TikTok, and YouTube Shorts: 9:16 H.264, `yuv420p`, 60 fps. Every social preset targets **24 Mbps**, and the encoder also caps the peak at that rate. That is the room fine particle detail needs while staying under the 25 Mbps ceiling those apps publish. A lower rate bakes blur into sparks and grit before the platform re-encodes the upload. About 100 seconds at 24 Mbps is 300 MB, which is the usual upload-size limit, so keep a particle reel shorter than that if the app rejects the file. `pad` keeps a square dome master round and centered. `social-reel-fill-60` crops the sides so the phone frame is full. `social-feed-60` is the 4:5 feed portrait. `social-square-60` is an even scale of a square master. Social presets set `-movflags +faststart`. Dome presets do not, and their bitrates stay at 100M–200M.
